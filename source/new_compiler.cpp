...
{
    auto makeLoadInstr = [](Compiler & comp, const IntermediateInstr * inInstr)
    {
        IntermediateInstr * newLoadInstr = newInstruction(comp, inInstr);
        const bool isGlobal = (inInstr->op == OpCode::StoreSetTypeGlobal);
...
static IntermediateInstr * emitBreak(Compiler & compiler, CodeGenContext & codeGen, const SyntaxTreeNode * root)
{
    MOON_ASSERT(codeGen.loopEndAnchor != nullptr);
    return newInstruction(compiler, OpCode::Jmp, codeGen.loopEndAnchor);
}

static IntermediateInstr * emitContinue(Compiler & compiler, CodeGenContext & codeGen, const SyntaxTreeNode * root)
{
...
static IntermediateInstr * emitCompoundBinaryOp(Compiler & compiler, CodeGenContext & codeGen, const SyntaxTreeNode * root)
{
    ...
    {
        storeOp = (!lhsIsArraySubscript ? OpCode::StoreGlobal : OpCode::StoreArraySubGlobal);
    ...
    if (root->getChild(1)->nodeType != SyntaxTreeNode::Type::ExprFuncCall)
    {
        ...
        return linkInstructions(lhs, rhs, binOpInstr, storeOpInstr, paramList);
    }
    else
    {
